<template>
  <div style="width: 90vw">
    <div class="page">
      ...
      <div class="info-box" v-if="list.length!=0">
        <div class="info-tab">
          ...
          <div class="tab-item" style="width: 30%">车辆数</div>
          <div class="tab-item" style="width: 30%;border-top-right-radius: 5px;">地址数</div>
        </div>
        <div class="info" v-for="item in list" :key="item.tel">
          ...
          <div class="tab-item" style="width: 30%" @click="popCar(item)">{{ item.cars.length }}</div>
          <div class="tab-item" style="width: 30%;border-bottom-right-radius: 5px;" @click="popAddress(item)">
    ...
    <div class="pop" v-show="pop">
      <div class="pop-box">
        <div class="pop-title" v-show="type==0">车辆列表</div>
        <div class="pop-title" v-show="type==1">地址列表</div>
        ...
        <div class="pop-item" v-for="e in addressList" :key="e.id" v-show="type==1"><div class="item-info">{{ e.address }}</div><Icon type="ios-power" style="margin-right: 5px" @click="deleteAddress(e)"/></div>
      </div>
...
export default {
  ...
  methods: {
    ...
    popCar(item) {
      ...
      this.type = 0
      this.id=item.id
      this.carList = item.cars
    },
    popAddress(item) {
      this.pop = true
      this.type = 1
      this.id=item.id
      this.addressList = item.addresses
    },
    deleteCar(i){
      this.$api.deletes('sdbl-auth/users/'+this.id+'/cars/'+i.id).then(res=>{
        console.log(res)
        if(res.status==204){
          this.listUser()
        }
    ...
    deleteAddress(e){
      this.$api.deletes('sdbl-auth/users/'+this.id+'/addresses/'+e.id).then(res=>{
        console.log(res)
        if(res.status==204){
          this.listUser()
        }
